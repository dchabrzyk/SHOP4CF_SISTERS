...
-- changeset Koza:2021206140000-1 rollbackSplitStatements:false
...
--rollback 	miRow "ModelInstances"%ROWTYPE;
--rollback 	emiRow "EquipmentModelInstances"%ROWTYPE;
--rollback BEGIN
...
--rollback 		-- create copy
--rollback 		WITH inserted_row AS (
--rollback 			INSERT INTO "StepResources" ("Id", "ScenarioId", "OrganizationId", "StepId", "ResourceId", "FixedQuantityValue", "AssignmentType", "AlternativesCategory", "FixedQuantityUnit", "QuantityPerCycleValue", "QuantityPerCycleUnit", "UsageType", "CapabilityRequirements", "ResourceType")
--rollback 			VALUES (uuid_generate_v4(), "scenarioId", tsrRow."OrganizationId", stepId, resourceId, tsrRow."FixedQuantityValue", tsrRow."AssignmentType", tsrRow."AlternativesCategory", tsrRow."FixedQuantityUnit", tsrRow."QuantityPerCycleValue", tsrRow."QuantityPerCycleUnit", tsrRow."UsageType", tsrRow."CapabilityRequirements", tsrRow."ResourceType")
--rollback 			RETURNING "Id"
--rollback 		)
--rollback 		INSERT INTO "IdMappings" ("OldId", "NewId")
...
--rollback 	END LOOP;
--rollback 	
--rollback 	-- ModelInstances
--rollback 	FOR miRow IN SELECT * FROM "ModelInstances" WHERE "ScenarioId" = productionScenarioId
--rollback 	LOOP
--rollback 		-- ModelInstance
--rollback 		WITH inserted_row AS (
--rollback 			INSERT INTO "ModelInstances" ("Id", "ScenarioId", "OrganizationId", "Schema", "Version", "Value", "ExternalId")
--rollback 			VALUES (uuid_generate_v4(), "scenarioId", miRow."OrganizationId", miRow."Schema", miRow."Version", miRow."Value", miRow."ExternalId")
--rollback 			RETURNING "Id"
--rollback 		)
--rollback 		INSERT INTO "IdMappings" ("OldId", "NewId")
--rollback 		SELECT miRow."Id", "Id" FROM inserted_row;
--rollback 
--rollback 	END LOOP;
--rollback 	
--rollback 	-- EquipmentModelInstances
--rollback 	FOR emiRow IN SELECT * FROM "EquipmentModelInstances" WHERE "ScenarioId" = productionScenarioId
--rollback 	LOOP
--rollback 		resourceId := null;
--rollback 		modelInstanceId := null;
--rollback 		
--rollback 		SELECT "NewId" INTO resourceId FROM "IdMappings" WHERE "OldId" = emiRow."EquipmentId";
--rollback 		SELECT "NewId" INTO modelInstanceId FROM "IdMappings" WHERE "OldId" = emiRow."ModelInstanceId";
--rollback 		
--rollback 		-- EquipmentModelInstance
--rollback 		
--rollback 		INSERT INTO "EquipmentModelInstances" ("EquipmentId", "ModelInstanceId", "ScenarioId")
--rollback 		VALUES (resourceId, modelInstanceId, "scenarioId");
--rollback 			
--rollback 	END LOOP;
--rollback END;
--rollback ';
