...
-- changeset Gchegosh:20220406000000-1
ALTER TABLE "StepResourceSpecs"
    ADD COLUMN "AlternativeResourceIds" uuid[] DEFAULT '{}';

-- changeset Gchegosh:20220406000000-2
-- comment aggregate AlternativeResourceIds for the equal "Alternative" StepResourceSpecs 
UPDATE "StepResourceSpecs" s1
SET "AlternativeResourceIds" =
        (SELECT array_agg(s2."ResourceId")
         FROM "StepResourceSpecs" s2
         WHERE s1."ResourceId" != s2."ResourceId"
           AND s1."StepId" = s2."StepId"
           AND s1."TenantId" = s2."TenantId"
           AND s1."ScenarioId" = s2."ScenarioId"
           AND s1."ResourceType" = s2."ResourceType"
           AND s1."AssignmentType" = s2."AssignmentType"
           AND s1."AlternativesCategory" = s2."AlternativesCategory"
           AND s1."UsageType" = s2."UsageType"
        )
WHERE s1."AssignmentType" = 2;

-- changeset Gchegosh:20220406000000-3
-- comment delete redundant "Alternative" entities
DELETE
FROM "StepResourceSpecs" s1 USING "StepResourceSpecs" s2
WHERE s1.CTID > s2.CTID
  ...
  AND s1."AlternativesCategory" = s2."AlternativesCategory"
  AND s1."UsageType" = s2."UsageType"
  AND s1."AssignmentType" = 2
  AND s1."AssignmentType" = s2."AssignmentType";

-- changeset Gchegosh:20220406000000-4
UPDATE "StepResourceSpecs" s1
SET "AlternativeResourceIds" = '{}'
WHERE "AlternativeResourceIds" IS NULL;

-- changeset Gchegosh:20220406000000-5
ALTER TABLE "StepResourceSpecs"
    DROP COLUMN "AlternativesCategory";

-- changeset Gchegosh:20220406000000-6
...
BEGIN
    ...
    LOOP
    -- NOTE: new AlternativeResourceIds, removed AlternativesCategory
    INSERT INTO "StepResourceSpecs" ("Id", "ScenarioId", "TenantId", "StepId", "ResourceId", "QuantityValue", "AssignmentType", "QuantityUnit", "UsageType", "CapabilityRequirements", "ResourceType", "ProcessingTime", "QuantityPerTime", "AlternativeResourceIds")
    VALUES (srsRow."Id", "scenarioId", srsRow."TenantId", srsRow."StepId", srsRow."ResourceId", srsRow."QuantityValue", srsRow."AssignmentType", srsRow."QuantityUnit", srsRow."UsageType", srsRow."CapabilityRequirements", srsRow."ResourceType", srsRow."ProcessingTime", srsRow."QuantityPerTime", srsRow."AlternativeResourceIds")
    ;
    END LOOP;
